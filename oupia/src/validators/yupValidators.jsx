...
export const schemaUser = yup.object().shape({
    ...
    identityNumber: yup.string().required('Số CMND/CCCD không được để trống').matches(/^\d+$/, 'Số CMND/CCCD phải là số'),
...
export const schemaMotel = yup.object().shape({
    ...
    phoneNumber: yup.string().required('Bắt buộc nhập số điện thoại').length(10, 'Số điện thoại phải có 10 chữ số.'),
